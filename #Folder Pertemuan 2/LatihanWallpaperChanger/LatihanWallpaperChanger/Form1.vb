...
Public Class Form1
    ...
    Private Sub clbWallpaper_SelectedIndexChanged(sender As Object, e As EventArgs) Handles clbWallpaper.SelectedIndexChanged
        If clbWallpaper.SelectedItem Is Nothing Then Exit Sub

        Dim imageLocation As String = IO.Path.Combine(txtPath.Text, clbWallpaper.SelectedItem.ToString)

        If cbPreview.Checked AndAlso IO.File.Exists(imageLocation) Then
            PictureBox1.Image = New Bitmap(imageLocation)
        Else
            MsgBox("File tidak ditemukan: " & imageLocation)
        End If

        arrayGambar.Clear()
        For Each item In clbWallpaper.CheckedItems
            Dim path As String = IO.Path.Combine(txtPath.Text, item.ToString)
            If IO.File.Exists(path) Then arrayGambar.Add(path)
        Next
    End Sub
...
SPIF_UPDATEINIFILE Or SPIF_SENDWININICHANGE)
    ...
    Private Sub AboutToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles AboutToolStripMenuItem.Click
        MsgBox("Wallpaper Changer v1.0 " & vbCrLf & "Created by Imanuel P. A Faot / 71220927" &
vbCrLf & "imanuel.putra@ti.ukdw.ac.id")
    End Sub
